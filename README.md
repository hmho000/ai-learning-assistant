...
### v1.1.0 - 2025-11-29 (Phase 4: Refinement)
- **✨ 新增功能**
  - **拖拽上传**：支持将 PDF 文件直接拖拽到上传区域。
  - **课程管理**：新增删除课程功能，可一键清理旧课程。
  - **自定义生成配置**：上传后可手动选择需要生成的章节，并自定义每章的题目数量（选择题/填空题）。
- **⚡ 优化**
  - **大文件处理**：优化了文件上传和解析流程，增强了对大文件（>50MB）的错误处理和反馈。
  - **UI 交互**：优化了上传页面的交互体验，增加了加载状态和错误提示。
- **🐛 修复**
  - 修复了部分 PDF 解析失败导致流程卡死的问题。

### v1.0.0 - 2025-11-28 (Initial Release)
- 完成从命令行脚本到 Web 应用的架构升级。
- 实现 PDF 解析、AI 出题、Markdown 渲染全流程。
- 前端支持阅读模式和交互式答题模式。
- 后端采用 FastAPI + SQLModel + SQLite 架构。
